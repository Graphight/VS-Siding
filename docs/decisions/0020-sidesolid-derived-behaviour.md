...
- **A torch hangs on glazing**, because a glazed wall seals and the override asks only that. Confirmed as vanilla's own split, not an oversight: `glass/full-plain.json` sets `sideopaque: false` and leaves `sidesolid` at its default true, so vanilla lets you hang a torch on a solid glass block, while `glass/pane.json` sets `sidesolid: { all: false }` and refuses. A glazed siding wall seals a room and dams water, so it is the full block and not the pane. It looks odd; vanilla looks equally odd, and matching it beats inventing a third rule.
