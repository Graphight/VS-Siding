...
Vanilla also asks the *clicked* block `IsReplacableBy` in the client's `OnBlockBuild`, to decide whether a click places into that cell or past its face; there a wall answers no except on its open side's inner face (a transpiled `ClickedIsReplacableBy`), so a torch on a wall's outer face or a chest on its top still lands beyond it, while a torch on the inner face hangs in the wall's cell even with a saw in the off hand, which skips `TryHost`.
